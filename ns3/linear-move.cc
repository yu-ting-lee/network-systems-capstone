...
int main(int argc, char *argv[]) {
  ...
  Simulator::Schedule(Seconds(stepsTime), &Movement, wirelessNodes.Get(0),
                      stepsSize, stepsTime);
...
void CheckLinkState(Ptr<Node> node) {
  ...
  if (DynamicCast<StaWifiMac>(clientMac)->IsAssociated()) {
    std::cout << "Client connects to AP "
              << DynamicCast<RegularWifiMac>(clientMac)->GetBssid()
              << std::endl;
  } else {
...
void Movement(Ptr<Node> node, double stepsSize, double stepsTime) {
  ...
  mobility->SetPosition(Vector(pos.x + stepsSize, pos.y, pos.z));
  pos = mobility->GetPosition();
  std::cout << "Current position: ( " << pos.x << ", " << pos.y << ", " << pos.z
            << " )" << std::endl;

  CheckLinkState(node);
  Simulator::Schedule(Seconds(stepsTime), &Movement, node, stepsSize,
                      stepsTime);
}
