...
TypeId StaWifiMac::GetTypeId(void) {
  static TypeId tid =
      TypeId("ns3::StaWifiMac")
          ...
          .AddAttribute("ActiveProbing",
                        ...
                        "the same time resulting in collisions. "
...
void StaWifiMac::DoInitialize(void) {
  NS_LOG_FUNCTION(this);
  StartScanning();
}
...
uint16_t StaWifiMac::GetAssociationId(void) const {
...
bool StaWifiMac::GetActiveProbing(void) const { return m_activeProbing; }

void StaWifiMac::SetWifiPhy(const Ptr<WifiPhy> phy) {
  NS_LOG_FUNCTION(this << phy);
  RegularWifiMac::SetWifiPhy(phy);
  m_phy->SetCapabilitiesChangedCallback(
      MakeCallback(&StaWifiMac::PhyCapabilitiesChanged, this));
}

void StaWifiMac::SetWifiRemoteStationManager(
...
void StaWifiMac::SendProbeRequest(void) {
  NS_LOG_FUNCTION(this);
  WifiMacHeader hdr;
...
void StaWifiMac::SendDisassociationMsg(void) {
  ...
  const uint8_t buffer[2] = {0x01, 0x00};
  Ptr<Packet> packet = Create<Packet>(buffer, 2);

  m_txop->Queue(packet, hdr);
}

void StaWifiMac::SendCfPollResponse(void) {
  NS_LOG_FUNCTION(this);
  NS_ASSERT(GetPcfSupported());
  m_txop->SendCfFrame(WIFI_MAC_DATA_NULL, GetBssid());
}

void StaWifiMac::TryToEnsureAssociated(void) {
  NS_LOG_FUNCTION(this);
  switch (m_state) {
    case ASSOCIATED:
      return;
...
void StaWifiMac::StartScanning(void) {
  NS_LOG_FUNCTION(this);
  ...
  if (GetActiveProbing()) {
    ...
    m_probeRequestEvent = Simulator::Schedule(
        m_probeRequestTimeout, &StaWifiMac::ScanningTimeout, this);
  } else {
    SetState(WAIT_BEACON);
    m_waitBeaconEvent = Simulator::Schedule(m_waitBeaconTimeout,
                                            &StaWifiMac::ScanningTimeout, this);
  }
}
...
void StaWifiMac::ScanningTimeout(void) {
  ...
  if (!m_candidateAps.empty()) {
    ...
    if (bestAp.m_activeProbing) {
      UpdateApInfoFromProbeResp(bestAp.m_probeResp, bestAp.m_apAddr,
                                bestAp.m_bssid);
...
void StaWifiMac::AssocRequestTimeout(void) {
  NS_LOG_FUNCTION(this);
  SetState(WAIT_ASSOC_RESP);
  SendAssociationRequest(false);
}

void StaWifiMac::MissedBeacons(void) {
  NS_LOG_FUNCTION(this);
...
void StaWifiMac::RestartBeaconWatchdog(Time delay) {
  ...
  if (Simulator::GetDelayLeft(m_beaconWatchdog) < delay &&
    ...
    m_beaconWatchdog =
        Simulator::Schedule(delay, &StaWifiMac::MissedBeacons, this);
  }
}

bool StaWifiMac::IsAssociated(void) const { return m_state == ASSOCIATED; }

bool StaWifiMac::IsWaitAssocResp(void) const {
...
void StaWifiMac::Enqueue(Ptr<Packet> packet, Mac48Address to) {
  ...
  if (GetQosSupported()) {
    hdr.SetNoOrder();  // explicitly set to 0 for the time being since HT
                       // control field is not yet implemented (set it to 1
                       // when implemented)
  }
...
void StaWifiMac::Receive(Ptr<WifiMacQueueItem> mpdu) {
  ...
  if (hdr->GetAddr3() == GetAddress()) {
    ...
    return;
             ...
             (hdr->GetAddr2() == GetBssid()) && hdr->IsCfPoll()) {
    SendCfPollResponse();
  }
  if (hdr->IsData()) {
    if (!IsAssociated()) {
    ...
    if (hdr->GetAddr2() != GetBssid()) {
      NS_LOG_LOGIC(
          "Received data frame not from the BSS we are associated with: "
          "ignore");
      NotifyRxDrop(packet);
      return;
    }
    ...
    return;
  } else if (hdr->IsProbeReq() || hdr->IsAssocReq() || hdr->IsReassocReq()) {
    ...
    if (cfParameterSet.GetCFPCount() == 0) {
      ...
      if (GetPcfSupported()) {
        ...
        m_low->DoNavStartNow(
            MicroSeconds(cfParameterSet.GetCFPDurRemainingUs()));
      }
    }
    ...
    if (m_phy->GetNBssMembershipSelectors() > 0 &&
        bssMembershipSelectorMatch == false) {
    ...
    if (goodBeacon && m_state == ASSOCIATED) {
      ...
      NS_ASSERT(removed);
      // std::cout << snrTag.Get () << std::endl;
      // std::cout << 10 * log10(snrTag.Get ()) << std::endl;

      if (10 * log10(snrTag.Get()) < m_snrThreshold) {
    ...
    if (m_state == WAIT_PROBE_RESP) {
      NS_LOG_DEBUG("Probe response received while scanning from "
                   << hdr->GetAddr2());
...
void StaWifiMac::UpdateCandidateApList(ApInfo newApInfo) {
  NS_LOG_FUNCTION(this << newApInfo.m_bssid << newApInfo.m_apAddr
                       << newApInfo.m_snr << newApInfo.m_activeProbing
                       << newApInfo.m_beacon << newApInfo.m_probeResp);
  // Remove duplicate ApInfo entry
  for (std::vector<ApInfo>::iterator i = m_candidateAps.begin();
       i != m_candidateAps.end(); ++i) {
  ...
  if (10 * log10(newApInfo.m_snr) < m_snrThreshold) {
    // std::cout << 10 * log10(newApInfo.m_snr) << std::endl;
    // std::cout << newApInfo.m_bssid << std::endl;
    return;
  }
...
void StaWifiMac::UpdateApInfoFromBeacon(MgtBeaconHeader beacon,
                                        Mac48Address apAddr,
                                        Mac48Address bssid) {
  ...
  if (GetQosSupported()) {
    ...
    if (edcaParameters.IsQosSupported()) {
      qosSupported = true;
      // The value of the TXOP Limit field is specified as an unsigned
      // integer, with the least significant octet transmitted first, in
      // units of 32 μs.
  ...
  if (GetHtSupported()) {
    ExtendedCapabilities extendedCapabilities =
        beacon.GetExtendedCapabilities();
    //
  }
  if (GetHeSupported()) {
    ...
    if (heCapabilities.GetSupportedMcsAndNss() != 0) {
      ...
      for (uint8_t i = 0; i < m_phy->GetNMcs(); i++) {
        ...
        if (mcs.GetModulationClass() == WIFI_MOD_CLASS_HE &&
            heCapabilities.IsSupportedRxMcs(mcs.GetMcsValue())) {
          m_stationManager->AddSupportedMcs(apAddr, mcs);
        }
      }
    }
...
void StaWifiMac::UpdateApInfoFromProbeResp(MgtProbeResponseHeader probeResp,
                                           Mac48Address apAddr,
                                           Mac48Address bssid) {
  ...
  if (GetErpSupported()) {
    ...
    for (uint8_t i = 0; i < m_phy->GetNModes(); i++) {
      ...
      if (mode.GetModulationClass() == WIFI_MOD_CLASS_ERP_OFDM &&
          rates.IsSupportedRate(mode.GetDataRate(m_phy->GetChannelWidth()))) {
        isErpAllowed = true;
        break;
      }
    }
...
void StaWifiMac::UpdateApInfoFromAssocResp(MgtAssocResponseHeader assocResp,
                                           Mac48Address apAddr) {
  ...
  if (GetHtSupported()) {
    ...
    for (uint8_t i = 0; i < m_phy->GetNMcs(); i++) {
      ...
      if (mcs.GetModulationClass() == WIFI_MOD_CLASS_HT &&
        ...
        m_stationManager->AddSupportedMcs(apAddr, mcs);
        // here should add a control to add basic MCS when it is
        // implemented
      }
    }
  }
  ...
  if (GetHtSupported()) {
    ExtendedCapabilities extendedCapabilities =
        assocResp.GetExtendedCapabilities();
    //
  }
  if (GetHeSupported()) {
    HeCapabilities heCapabilities = assocResp.GetHeCapabilities();
...
SupportedRates StaWifiMac::GetSupportedRates(void) const {
...
CapabilityInformation StaWifiMac::GetCapabilities(void) const {
  CapabilityInformation capabilities;
  capabilities.SetShortPreamble(m_phy->GetShortPhyPreambleSupported() ||
                                GetErpSupported());
...
void StaWifiMac::SetState(MacState value) {
...
void StaWifiMac::SetEdcaParameters(AcIndex ac, uint32_t cwMin, uint32_t cwMax,
                                   uint8_t aifsn, Time txopLimit) {
...
void StaWifiMac::PhyCapabilitiesChanged(void) {
  NS_LOG_FUNCTION(this);
  if (IsAssociated()) {
